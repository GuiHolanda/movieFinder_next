...
import { useListContext } from '../helpers/AppContext';
...
export default function FavoriteButton({ movie }: { movie: IMovie }) {
  ...
  function handleButtonClick() {
    if (!favoriteList.some(listMovie => listMovie.Title === movie.Title)) {
      ...
      setButtonStatus('bg-red-600 text-white border-none');
    } else {
  ...
  return (
    <button
      ...
      type="button"
      className={`flex h-fit border ${buttonStatus} border-gray-600 p-1 rounded-md`}
    ...
    >
      ...
      <svg
        ...
        height="16"
        fill="currentColor"
        className="bi bi-heart self-center ml-2"
        viewBox="0 0 16 16"
      >
